...
void displayIndex(string data_path,string index_path, bool avg = false)
{
	...
	for (size_t i = 0; i < cnt; i++)
	{
		cout << "|" << setw(5) << I[i] + 1 << "|"
			...
			<< setw(13) << stud[I[i]].mark_informatic << "|";
		if (avg)
...
void RealSort(string path)
{
	...
	for (size_t i = 0; i < cnt; i++)
	{
		for (size_t j = 0; j < cnt - i - 1; j++)
		{
			...
			if (j0 < j1 || j0 == j1 && stud[j].mark_informatic < stud[j + 1].mark_informatic
				|| j0 == j1 && stud[j].mark_informatic == stud[j + 1].mark_informatic && stud[j].surname < stud[j + 1].surname)
	...
	if (f.is_open())
	{
		...
		for (size_t i = 0; i < cnt; i++)
		{
			...
			f.write((char*)&stud[i].mark_informatic, sizeof(double));
		}
	}
	else
	...
	for (size_t i = 1; i < cnt; i++)
	{
		...
		for (j = i - 1;
			j >= 0 && (avgMark(stud[I[j]]) < avgMark(stud[value]) ||
				avgMark(stud[I[j]]) == avgMark(stud[value]) && stud[j].mark_informatic < stud[j + 1].mark_informatic ||
				avgMark(stud[I[j]]) == avgMark(stud[value]) && stud[j].mark_informatic == stud[j + 1].mark_informatic && stud[j].surname < stud[j + 1].surname);
			j--)
		{
			I[j + 1] = I[j];
		}
...
int BinSearch(string path, string surname, double avg, double last_mark)
{
	...
	if (f.is_open()) {
		...
		for (size_t i = 0; i < cnt; i++)
		{
			...
			f.read((char*)&stud.mark_informatic, sizeof(double));
	...
	{
		m = (l + r) / 2;
		if (s[m].surname == surname && avgMark(s[m]) * 100 == avg*100 && s[m].mark_informatic == last_mark) {
			delete[] s;
			return m;
		}
		int a = avgMark(s[m]) * 100;
		if (a > avg * 100 || a == avg * 100 && s[m].mark_informatic > last_mark || a == avg * 100 && s[m].mark_informatic == last_mark && s[m].surname > surname)
			l = m + 1;
		else
			r = m - 1;
	} while (l < r);
